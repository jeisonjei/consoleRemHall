...
namespace wasmSmokeMan.Shared.RemoveHall
{
    ...
    public class NetPart
    {
        ...
        public double dpDuct
        {
            get
            {
                ...
                throw new ArithmeticException(
                    "Нельзя вычислить dP, так как не задан тип воздуховода - круглый или прямоугольный");
            }
        }
        ...
        {
            get { return FlowStart / (Duct.Area * Fluid.Density); }
        }
        ...
        public double Gda
        {
            get
            {
                ...
                return 3.556 * Math.Pow(10, -5) * Fluid.Density * (Math.Pow((PressureStart + PressureEnd) / 2, 0.65)) *
                       (Duct.Area / Duct.HidraulicDiameter.ToMeters()) *
                       (Floor.Height + AdditionalDuctLength);
            }
        }

        //подсосы воздуха через клапан на участке
        public double Gdpa
        {
            get
            {
                //формула 85
                if (Valve != null) return Valve.Area * (Math.Pow(PressureEnd / Valve.Sdpsm, 0.5));
                return 0;
            }
        }
        ...
        public double TsmEnd
        {
            get
            {
                ...
                return (FlowStart * TsmStart + Gda * Climate.TempInside.ToKelvin()+Gdpa*Climate.TempInside.ToKelvin()) / FlowEnd;
            }
        }
        ...
        public NetPart(Duct duct, /*воздуховод*/
            double additionalDuctLength, /*добавочная длина, если воздуховод не проходит на этаже прямо снизу вверх*/
            double ksiSum /*сумма КМС на участке*/, Floor floor, Climate climate, Valve valve /*клапан на этаже*/)
        {
            Duct = duct;
            AdditionalDuctLength = additionalDuctLength;
            KsiSum = ksiSum;
